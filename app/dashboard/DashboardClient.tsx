...
const STATUS_OPTIONS: Record<string, string[]> = {
  status: [
    "", "Prospect Client", "New Client", "Existing Client", "Referral Client", "Initiated", "In-Progress", "On-Hold", "Closed", "Completed",
  ],
  ...
  client_status: [
    "", "New Client", "Initiated", "Interested", "In-Progress", "Closed", "On Hold", "Purchased", "Re-Opened", "Completed",
  ],
  bop_status: [
    "", "Presented", "Business", "Client", "In-Progress", "On-Hold", "Clarification", "Not Interested", "Completed", "Closed",
  ],
...
export default function DashboardClient() {
  ...
  // Logo state (we only toggle opacity, never remove or resize the box)
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!data.session) {
          // With server-side guard in page.tsx, we should always be authenticated here.
          // This is a secondary safety redirect only in rare race conditions.
  ...
  async function loadBusinessPage(nextPage: number) {
    ...
    try {
      ...
      if (search)
        countQuery = countQuery.or(
          `client_name.ilike.%${search}%,associate_name.ilike.%${search}%,policy_number.ilike.%${search}%`
        );
      const { count, error: cErr } = await countQuery;
      if (cErr) throw cErr;
      setBusinessTotal(count ?? 0);
      ...
      setBusinessPageJump(String(nextPage + 1));
    } catch (e: any) {
      setError(e?.message ?? "Failed to load business records");
    } finally {
      setBusinessLoading(false);
    }
  }

  async function updateBusinessCell(id: string, key: string, rawValue: string) {
    setSavingId(id);
    setError(null);
    try {
      const supabase = getSupabase();
      ...
      payload[key] = isDateTime
        ...
        ? rawValue
        : null;
      const { error } = await supabase.from("client_business").update(payload).eq("id", id);
      if (error) throw error;
      const patch = (prev: Row[]) => prev.map((r) => (String(r.id) === String(id) ? { ...r, [key]: payload[key] } : r));
      setBusinessRecords(patch);
    } catch (e: any) {
      setError(e?.message ?? "Update failed");
      throw e;
    } finally {
      ...
      <div className="max-w-[1600px] mx-auto p-4 space-y-4">
        ...
        <Card title="Business 💼">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-2">
            <div className="flex flex-col md:flex-row md:items-center gap-2 w-full">
              <input
                className="w-96 border border-slate-300 px-3 py-2"
                placeholder="Search client name, associate, policy #"
                value={businessQ}
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 border border-slate-300 px-4 py-3 bg-white">
                ...
                <input
                  type="number"
                  min={1}
                  max={businessTotalPages}
                  className="w-20 border border-slate-300 px-3 py-2 text-sm"
                  value={businessPageJump}
...
function ExcelTableEditable({
  ...
  const columns = useMemo(() => {
    ...
    const main = keys.map((k) => {
      ...
      const defaultW =
        k === "created_at" ? 120 : isDateTime ? 220 : isDateOnly ? 180 : k.toLowerCase().includes("email") ? 240 : WRAP_KEYS.has(k) || READONLY_LIST_COLS.has(k) ? 260 : 160;
      const sortable =
                    ...
                    <td key={c.id} className={`border border-slate-300 px-2 py-2 ${shouldHighlight(k, r) ? "bg-yellow-200" : ""}`} style={style}>
                      ...
                      >
                        {statusOptions.map((opt, idx) => (
                          <option key={`${k}:${idx}:${opt}`} value={opt}>{opt || "—"}</option>
                        ))}
                      </select>
                    </td>
                  );

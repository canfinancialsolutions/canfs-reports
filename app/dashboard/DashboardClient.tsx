...
import { createBrowserSupabase } from '/lib/client'
...
function usePersistentBoolean(key: string, initial: boolean) {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem(key); if (raw != null) setVal(raw === "1"); } catch {}
...
export default function DashboardClient() {
  ...
  async function logout() { try { const supabase = getSupabase(); await supabase.auth.signOut(); } finally { window.location.href = "/"; } }
  ...
  async function fetchTrends() {
    ...
    } catch (e: any) { setError(e?.message ?? "Failed to load trends"); } finally { setTrendLoading(false); }
  }
  ...
  const toggleAllCards = () => { const target = !allVisibleWithBusiness; setTrendsVisible(target); setUpcomingVisible(target); setProgressVisible(target); setRecordsVisible(target); setBusinessVisible(target); };
  ...
  return (
    <div className={`${inter.className} min-h-screen`}>
      <div className="max-w-[1600px] mx-auto p-4 space-y-4">
        ...
        <Card title="Trends 📊">
          <div className="mb-2">
            <Button type="button" variant="secondary" onClick={() => setTrendsVisible((v) => !v)}>{trendsVisible ? "Hide 📊" : "Show 📊"}</Button>
          </div>
          {trendsVisible ? (
            <>
        ...
        <thead className="sticky top-0 bg-slate-100 z-20">
          <tr className="text-left text-xs font-semibold text-black">
            {(columns as any).map((c: any, colIndex: number) => {
              ...
              const style: CSSProperties = { width: w, minWidth: w, maxWidth: w, position: isSticky ? "sticky" : undefined, left: isSticky ? stickyLeftPx(colIndex) : undefined, top: 0, zIndex: isSticky ? 50 : 20, background: isSticky ? "#f1f5f9" : undefined };
            ...
            <tr key={String(r.id ?? ridx)} className={`hover:bg-slate-50 ${r.client_status === "New Client" ? "bg-[#B1FB17]" : r.client_status === "Interested" ? "bg-[#728FCE]" : r.client_status === "In-Progress" ? "bg-[#ADDFFF]" : r.client_status === "Closed" ? "bg-[#E6BF83]" : r.client_status === "On Hold" ? "bg-[#C9BE62]" : r.client_status === "Completed" ? "bg-[#3CB371] text-black" : ""}`}>
              {(columns as any).map((c: any, colIndex: number) => {
                ...
                if ((c as any).kind === "extra") {
                  ...
                  return (
                    <td key={c.id} className={`border border-slate-300 px-2 py-2 whitespace-nowrap font-semibold text-black`} style={style}>{v}</td>
                  );
                }

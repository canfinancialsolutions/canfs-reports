...
// Stable font to avoid font-swap reflow
...
function toggleSort(cur: { key: SortKey; dir: SortDir }, k: SortKey) {
  const DESC_FIRST = new Set<SortKey>(["CalledOn", "BOP_Date", "Followup_Date", "issue_date", "submit_date", "created_at"]);
  if (cur.key !== k) {
...
function toggleProgressSort(cur: { key: ProgressSortKey; dir: SortDir }, k: ProgressSortKey) {
  const DESC_FIRST = new Set<ProgressSortKey>(["last_call_date", "last_bop_date", "last_followup_date"]);
  if (cur.key !== k) return { key: k, dir: (DESC_FIRST.has(k) ? "desc" : "asc") as SortDir };
  return { key: k, dir: cur.dir === "asc" ? ("desc" as SortDir) : ("asc" as SortDir) };
}

function useColumnResizer() {
  const [widths, setWidths] = useState<Record<string, number>>({});
  const resizeRef = useRef<{ colId: string; startX: number; startW: number; minW: number } | null>(null);
...
// ---- Persisted UI toggles to avoid "fresh" state after navigation ----
...
export default function DashboardClient() {
  ...
  const [recordsVisible, setRecordsVisible]   = usePersistentBoolean("vis_records", false);
  const [trendsVisible, setTrendsVisible]     = usePersistentBoolean("vis_trends", false);
  const [upcomingVisible, setUpcomingVisible] = usePersistentBoolean("vis_upcoming", false);
  const [progressVisible, setProgressVisible] = usePersistentBoolean("vis_progress", false);
  const [businessVisible, setBusinessVisible] = usePersistentBoolean("vis_business", false);

  // Business Card
  const [businessQ, setBusinessQ] = useState("");
  const [businessRecords, setBusinessRecords] = useState<Row[]>([]);
  const [businessTotal, setBusinessTotal] = useState(0);
  const [businessPage, setBusinessPage] = useState(0);
  const [businessPageJump, setBusinessPageJump] = useState("1");
  const [businessLoading, setBusinessLoading] = useState(false);
  const [sortBusiness, setSortBusiness] = useState<{ key: SortKey; dir: SortDir }>({ key: "issue_date", dir: "desc" });

  // Logo state (opacity only)
  ...
  useEffect(() => { loadPage(0); }, [sortAll.key, sortAll.dir]);
  useEffect(() => { if (upcoming.length) fetchUpcoming(); }, [sortUpcoming.key, sortUpcoming.dir]);
  useEffect(() => { const id = setTimeout(() => loadPage(0), 300); return () => clearTimeout(id); }, [q]);
  useEffect(() => { if (businessVisible) loadBusinessPage(0); }, [businessVisible, sortBusiness.key, sortBusiness.dir]);
  useEffect(() => { if (!businessVisible) return; const t = setTimeout(() => loadBusinessPage(0), 300); return () => clearTimeout(t); }, [businessQ]);
  ...
  async function logout() {
    try { const supabase = getSupabase(); await supabase.auth.signOut(); } finally { window.location.href = "/"; }
  }
  ...
  async function fetchTrends() {
    ...
    try {
      ...
      const days: string[] = []; const callsDay = new Map<string, number>(); const bopsDay = new Map<string, number>(); const fuDay = new Map<string, number>();
      for (let i = 0; i < 60; i++) { const d = addDays(startDaily, i); const key = format(d, "yyyy-MM-dd"); days.push(key); callsDay.set(key, 0); bopsDay.set(key, 0); fuDay.set(key, 0); }
      const bumpDay = (dateVal: any, map: Map<string, number>) => { if (!dateVal) return; const d = parseISO(String(dateVal)); if (!isValid(d)) return; const k = format(d, "yyyy-MM-dd"); if (map.has(k)) map.set(k, (map.get(k) ?? 0) + 1); };
      ...
      const months: string[] = []; const callsMonth = new Map<string, number>(); const bopsMonth = new Map<string, number>(); const fuMonth = new Map<string, number>();
      for (let i = 0; i < 12; i++) { const mDate = addMonths(startMonth, i); const key = format(mDate, "yyyy-MM"); months.push(key); callsMonth.set(key, 0); bopsMonth.set(key, 0); fuMonth.set(key, 0); }
      ...
      const bumpMonth = (dateVal: any, map: Map<string, number>) => { if (!dateVal) return; const d = parseISO(String(dateVal)); if (!isValid(d)) return; const k = format(d, "yyyy-MM"); if (map.has(k)) map.set(k, (map.get(k) ?? 0) + 1); };
  ...
  async function fetchProgressSummary() {
    ...
    } catch (e: any) { setError(e?.message ?? "Failed to load Client Progress Summary"); }
  ...
  async function updateCell(id: string, key: string, rawValue: string) {
    ...
    } catch (e: any) { setError(e?.message ?? "Update failed"); throw e; }
  ...
  async function loadBusinessPage(nextPage: number) {
    setBusinessLoading(true); setError(null);
    try {
      const supabase = getSupabase();
      const search = businessQ.trim();
      let countQuery = supabase.from("client_business").select("id", { count: "exact", head: true });
      if (search) countQuery = countQuery.or(`client_name.ilike.%${search}%,associate_name.ilike.%${search}%,policy_number.ilike.%${search}%`);
      const { count, error: cErr } = await countQuery; if (cErr) throw cErr; setBusinessTotal(count ?? 0);

      const from = nextPage * ALL_PAGE_SIZE; const to = from + ALL_PAGE_SIZE - 1;
      let dataQuery = supabase.from("client_business").select("*").range(from, to);
      if (search) dataQuery = dataQuery.or(`client_name.ilike.%${search}%,associate_name.ilike.%${search}%,policy_number.ilike.%${search}%`);
      dataQuery = applyBusinessSort(dataQuery, sortBusiness);
      const { data, error } = await dataQuery; if (error) throw error;
      setBusinessRecords(data ?? []); setBusinessPage(nextPage); setBusinessPageJump(String(nextPage + 1));
    } catch (e: any) { setError(e?.message ?? "Failed to load business records"); }
  ...
  const allVisibleWithBusiness = trendsVisible && upcomingVisible && progressVisible && recordsVisible && businessVisible;
  const toggleAllCards = () => {
    const target = !allVisibleWithBusiness;
      ...
      <div className="max-w-[1600px] mx-auto p-4 space-y-4">
        ...
        <header className="flex items-center justify-between gap-2 h-16">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" variant="secondary" onClick={toggleAllCards}>{allVisibleWithBusiness ? "Hide All" : "Show All"}</Button>
        ...
        <Card title="Upcoming Meetings📣">
          <div className="grid md:grid-cols-5 gap-3 items-end">
            ...
            <div className="flex gap-2 md:col-span-3">
              <Button type="button" variant="secondary" onClick={() => fetchUpcoming()}><b>➡️</b></Button>
              <Button type="button" variant="secondary" onClick={() => { const today = new Date(); setRangeStart(format(today, "yyyy-MM-dd")); setRangeEnd(format(addDays(today, 30), "yyyy-MM-dd")); fetchUpcoming(); }} disabled={upcomingLoading}>{upcomingLoading ? "Refreshing…" : "🔄"}</Button>
              <Button type="button" variant="secondary" onClick={exportUpcomingXlsx} disabled={upcoming.length === 0}>📤</Button>
              <Button type="button" variant="secondary" onClick={() => setUpcomingVisible((v) => !v)}><span className={upcomingVisible ? "text-black" : undefined}>{upcomingVisible ? "Hide🗂️" : "Show🗂️"}</span></Button>
            </div>
          </div>
          ...
          {upcomingVisible && (
            <ExcelTableEditable
              ...
              savingId={savingId}
              onUpdate={updateCell}
              preferredOrder={["created_at","status","first_name","last_name","interest_type","business_opportunities","wealth_solutions","CalledOn","BOP_Date","BOP_Status","Followup_Date","FollowUp_Status","Product","Comment","Remark","client_status","phone","email","spouse_name","date_of_birth","children","city","state","profession","work_details","immigration_status","referred_by","preferred_days","preferred_time"]}
        ...
        <Card title="Client Progress Summary📑">
          <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
            ...
            <Button type="button" variant="secondary" onClick={() => { setProgressFilter(""); fetchProgressSummary().then(() => setProgressVisible(true)); }} disabled={progressLoading}>{progressLoading ? "Loading…" : "🔄"}</Button>
            <Button type="button" variant="secondary" onClick={() => setProgressVisible((v) => !v)}>{progressVisible ? "Hide🗂️" : "Show🗂️"}</Button>
          ...
          {progressVisible && (<>
            <ProgressSummaryTable rows={progressSlice} sortState={progressSort} onSortChange={(k) => setProgressSort((cur) => toggleProgressSort(cur, k))} />
            <div className="mt-2 text-xs text-black">Page <b>{progressPageSafe + 1}</b> of <b>{progressTotalPages}</b> • showing {PROGRESS_PAGE_SIZE} per page</div>
          </>)}
        </Card>
        ...
        <Card title="Clients List 🧑🏻‍🤝‍🧑🏻">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-2">
            <div className="flex flex-col md:flex-row md:items-center gap-2 w-full">
              ...
              <Button type="button" variant="secondary" onClick={() => setRecordsVisible((v) => !v)}>{recordsVisible ? "Hide🗂️" : "Show🗂️"}</Button>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 border border-slate-300 px-4 py-3 bg-white">
                ...
                <input type="number" min={1} max={Math.max(1, Math.ceil((total ?? 0) / ALL_PAGE_SIZE))} className="w-20 border border-slate-300 px-3 py-2 text-sm" value={pageJump} onChange={(e) => setPageJump(e.target.value)} />
                <Button type="button" variant="secondary" onClick={() => { const n = Number(pageJump); if (!Number.isFinite(n)) return; const p = Math.min(Math.max(1, Math.floor(n)), Math.max(1, Math.ceil((total ?? 0) / ALL_PAGE_SIZE))); loadPage(p - 1); }} disabled={loading || Math.max(1, Math.ceil((total ?? 0) / ALL_PAGE_SIZE)) <= 1}>➡️</Button>
              </div>
        ...
        <Card title="Business 💼">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-2">
            <div className="flex flex-col md:flex-row md:items-center gap-2 w-full">
              <input className="w-96 border border-slate-300 px-3 py-2" placeholder="Search client name, associate, policy #" value={businessQ} onChange={(e) => setBusinessQ(e.target.value)} />
              ...
              <Button type="button" variant="secondary" onClick={() => setBusinessVisible((v) => !v)}>{businessVisible ? "Hide🗂️" : "Show🗂️"}</Button>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 border border-slate-300 px-4 py-3 bg-white">
                <span className="text-xs font-semibold text-black">Go Page</span>
                <input type="number" min={1} max={businessTotalPages} className="w-20 border border-slate-300 px-3 py-2 text-sm" value={businessPageJump} onChange={(e) => setBusinessPageJump(e.target.value)} />
                <Button type="button" variant="secondary" onClick={() => { const n = Number(businessPageJump); if (!Number.isFinite(n)) return; const p = Math.min(businessTotalPages, Math.max(1, Math.floor(n))); loadBusinessPage(p - 1); }} disabled={businessLoading}>➡️</Button>
              </div>
          ...
          {businessVisible && (
            <ExcelTableEditable
              ...
              savingId={savingId}
              onUpdate={updateBusinessCell}
              extraLeftCols={[]}
              maxHeightClass="max-h-[560px]"
              sortState={sortBusiness}
              onSortChange={(k) => setSortBusiness((cur) => toggleSort(cur, k))}
              stickyLeftCount={0}
              viewOnlyPopupKeys={new Set()}
            />
          )}
...
function ProgressSummaryTable(
  { rows, sortState, onSortChange }:
  { rows: Row[]; sortState: { key: ProgressSortKey; dir: SortDir }; onSortChange: (k: ProgressSortKey) => void; }
) {
        ...
        <thead className="sticky top-0 bg-slate-100 z-20">
          <tr className="text-left text-xs font-semibold text-black">
            {cols.map((c, idx) => {
              ...
              const style: CSSProperties = { width: w, minWidth: w, maxWidth: w, position: isSticky ? "sticky" : undefined, left: isSticky ? stickyLeftPx(idx) : undefined, top: 0, zIndex: isSticky ? 40 : 20, background: isSticky ? "#f1f5f9" : undefined };
              return (
          ...
          {rows.map((r, ridx) => (
            <tr key={String((r as any).clientid ?? ridx)} className="hover:bg-slate-50">
              {cols.map((c, idx) => {
                const w = getW(c.id, c.defaultW);
                const isSticky = idx === 0;
                const style: CSSProperties = { width: w, minWidth: w, maxWidth: w, position: isSticky ? "sticky" : undefined, left: isSticky ? stickyLeftPx(idx) : undefined, zIndex: isSticky ? 10 : 1, background: isSticky ? "#ffffff" : undefined, verticalAlign: "middle" };
                let v = "—"; let tdClass = "border border-slate-300 px-2 py-2 whitespace-nowrap";
                ...
                return (
                  <td key={c.id} className={`${tdClass} ${isSticky ? "font-semibold text-black" : ""}`} style={style}>{v}</td>
                );
              })}
            </tr>
          ))}
...
function ExcelTableEditable({
  ...
  const sortIcon = (k?: SortKey) => {
    if (!k) return null; if (sortState.key !== k) return <span className="ml-1 text-black">↕</span>;
    return <span className="ml-1 text-black">{sortState.dir === "asc" ? "↑" : "↓"}</span>;
  };
  ...
  const keys = useMemo(() => {
    ...
    const set = new Set(baseKeys); const ordered: string[] = [];
  ...
  const columns = useMemo(() => {
    ...
    const main = keys.map((k) => {
      ...
      const sortable =
        k === "created_at" ? ("created_at" as SortKey) :
        k === "BOP_Date" ? ("BOP_Date" as SortKey) :
        k === "BOP_Status" ? ("BOP_Status" as SortKey) :
        k === "Followup_Date" ? ("Followup_Date" as SortKey) :
        ...
        k === "associate_name" ? ("associate_name" as SortKey) :
        k === "policy_number" ? ("policy_number" as SortKey) :
        k === "amount" ? ("amount" as SortKey) :
        k === "bill_amount" ? ("bill_amount" as SortKey) :
        undefined;
        ...
        <thead className="sticky top-0 bg-slate-100 z-20">
          <tr className="text-left text-xs font-semibold text-black">
            {(columns as any).map((c: any, colIndex: number) => {
              ...
              const style: CSSProperties = { width: w, minWidth: w, maxWidth: w, position: isSticky ? "sticky" : undefined, left: isSticky ? stickyLeftPx(colIndex) : undefined, top: 0, zIndex: isTopLeft ? 50 : 20, background: isSticky ? "#f1f5f9" : undefined };
          ...
          {rows.map((r, ridx) => (
            <tr key={String(r.id ?? ridx)} className={`hover:bg-slate-50 ${r.client_status === "New Client" ? "bg-[#B1FB17]" : r.client_status === "Interested" ? "bg-[#728FCE]" : r.client_status === "In-Progress" ? "bg-[#ADDFFF]" : r.client_status === "Closed" ? "bg-[#E6BF83]" : r.client_status === "On Hold" ? "bg-[#C9BE62]" : r.client_status === "Completed" ? "bg-[#3CB371] text-black" : ""}`}>
              {(columns as any).map((c: any, colIndex: number) => {
                const w = getW(c.id, c.defaultW ?? 160);
                const isSticky = colIndex < stickyLeftCount;
                const style: CSSProperties = { width: w, minWidth: w, maxWidth: w, position: isSticky ? "sticky" : undefined, left: isSticky ? stickyLeftPx(colIndex) : undefined, zIndex: isSticky ? 10 : 1, background: isSticky ? "#ffffff" : undefined };
                ...
                if (k === "created_at") {
                  ...
                  return (
                    <td key={c.id} className={`border border-slate-300 px-2 py-2 whitespace-nowrap ${shouldHighlight(k, r) ? "bg-yellow-200" : ""}`} style={style}>{v}</td>
                  );
                }
                const cellId = `${r.id}:${k}`;
                ...
                if (READONLY_LIST_COLS.has(k)) {
                  const cellIdList = `${r.id}:${k}`; const items = asListItems(r[k]); const display = items.join(", "); const showPopup = openCell === cellIdList;
                  return (
                      ...
                      <div className="relative">
                        ...
                        {showPopup && (
                          <div className="absolute left-0 top-full mt-1 w-72 max-w-[70vw] bg-white border border-slate-500 shadow-lg z-30">
                            ...
                            <ul className="max-h-48 overflow-auto">{(items.length ? items : ["(empty)")).map((x, i) => (<li key={i} className="px-2 py-1 text-sm border-b border-slate-100">{x}</li>))}</ul>
                ...
                if (WRAP_KEYS.has(k) && viewOnlyPopupKeys.has(k)) {
                  const cellIdView = `${r.id}:${k}`; const showPopup = openCell === cellIdView; const baseVal = String(getCellValueForInput(r, k));
                  return (
                    <td key={c.id} className={`border border-slate-300 px-2 py-2 align-top ${shouldHighlight(k, r) ? "bg-yellow-200" : ""}`} style={style}>
                      <div className="relative">
